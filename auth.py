...
def request_otp(telegram_id, email) -> tuple[bool, str]:
    ...
    if email in ("<email>", "email", "email@company.com", "<email@company.com>"):
        return False, "⚠️ Please enter your actual Odoo user email (e.g., /register nhatlovely2017@gmail.com)"
    try:
    ...
    if not users:
        return False, f"❌ Email '{email}' does not exist in Odoo. Please check your account email."
    if not users[0].get("active", True):
        return False, f"🚨 Account '{email}' has been disabled in Odoo."
    import random
    ...
    # FALLBACK METHOD: When email delivery fails or times out,
    ...
    return True, (
        ...
        f"👉 Type: `/verify {otp}` to complete account linking."
    )
...
def bind_direct(telegram_id, email) -> tuple[bool, str]:
    ...
    email = email.strip("<>\"' ").lower()
    if email in ("<email>", "email", "email@company.com", "<email@company.com>"):
        return False, "⚠️ Please enter your actual Odoo user email (e.g., /bind nhatlovely2017@gmail.com)"
    try:
        users = _odoo.search_read("res.users", ["|", ["login", "=ilike", email], ["email", "=ilike", email]],
                                  ["id", "name", "login", "active", "email"], 1)
    except Exception as e:
